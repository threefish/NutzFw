...
!function (i) {
    ...
    var m = i(window),
        v = i(document),
        f = "viewer",
        g = parent.document.createElement(f),
        w = "viewer-fixed",
        p = "viewer-open",
        b = "viewer-show",
        y = "viewer-hide",
    ...
    u.prototype = {
        constructor: u,
        // key 为索引值 ,value 为 图片的id
        // active 是当前活动的 图片的 index 索引 (默认为0)
        imgIds: {"active": 0, 'id': {}},
        init: function () {
            var $that = this,
                t = this.options,
            ...
            n.each(function (index, _element) {
                $that.imgIds.id[index] = i(_element).data("uuid") || "";
            });
            $that.show();
        }, ready: function () {
            ...
            switch (s) {
                ...
                case "mix":
                    ...
                    if (this.options && typeof this.options .closeFun != 'undefined' && /.*function.*/ig.test(Object.prototype.toString.call(this.options.closeFun))) {
            ...
            if (e.height > window.screen.availHeight) {
                e.height = window.screen.availHeight;
            }
            ...
            if (this.isFulled && t.keyboard) switch (e) {
                case 27:
                    ...
                    if (t && typeof t.closeFun != 'undefined' && /.*function.*/ig.test(Object.prototype.toString.call(t.closeFun))) {
                        t.closeFun();
                    }
                    break;
                case 32:
            ...
            o.opt = this.options;
            o.s = (t == 1);
            o.url = o.s ? o.opt.downloadItemUrl : o.opt.downloadAllUrl;
            if (!o.opt || !o.url || o.url == 'undefined' || o.opt.url == 'null') {
                return;
            }
            o.id = "from@id".replace(/@id/gi, Date.now());
            o.from = document.createElement("form") , o.input;
            o.from.setAttribute("action", o.url);
            o.from.setAttribute("id", o.id);
            o.from.setAttribute("method", "POST");
            o.from.setAttribute("target", "blank");
            o.input = document.createElement("input");
            o.input.setAttribute("name", "ids");
            o.input.setAttribute("value", o.s ? this.imgIds.id[this.imgIds.active] : o.opt.ids);
            o.from.appendChild(o.input);
            this.$body.append(o.from);
            i("#" + o.id).submit().remove();
        },
    }, u.DEFAULTS = {
        inline: !1,
        button: !0,
        ...
        isPlay: !0,
        // 下载单个文件的url地址
        downloadItemUrl: null
    },
        // u.TEMPLATE = '<div class="viewer-container"><div class="viewer-canvas"></div><div class="viewer-footer"><div class="viewer-title"></div><ul class="viewer-toolbar"><li class="viewer-zoom-in" data-action="zoom-in" title="放大"></li><li class="viewer-zoom-out" data-action="zoom-out" title="缩小"></li><li class="viewer-one-to-one" data-action="one-to-one" title="还原"></li><li class="viewer-reset" data-action="reset" title="重置"></li><li class="viewer-prev" data-action="prev" title="上一个"></li><li class="viewer-play" data-action="play" title="放映幻灯片"></li><li class="viewer-next" data-action="next" title="下一个"></li><li class="viewer-rotate-left" data-action="rotate-left" title="逆时针旋转"></li><li class="viewer-rotate-right" data-action="rotate-right" title="顺时针旋转"></li><li class="viewer-flip-horizontal" data-action="flip-horizontal" title="水平翻转"></li><li class="viewer-flip-vertical" data-action="flip-vertical" title="垂直翻转"></li><li class="viewer-download" data-action="download" title="下载浏览图片"></li></ul><div class="viewer-navbar"><ul class="viewer-list"></ul></div></div><div class="viewer-tooltip"></div><div class="viewer-button" data-action="mix"></div><div class="viewer-player"></div></div>',
        u.TLATE = (function () {
            var htmlArrays = [];
            htmlArrays.push('var stringBufStr = \'');
            htmlArrays.push('<div class="viewer-container">');
            htmlArrays.push('<div class="viewer-canvas"></div>');
            htmlArrays.push('<div class="viewer-footer">');
            htmlArrays.push('<div class="viewer-title"></div>');
            htmlArrays.push('<ul class="viewer-toolbar">');
            htmlArrays.push('<li class="viewer-zoom-in" data-action="zoom-in" title="放大"></li>');
            htmlArrays.push('<li class="viewer-zoom-out" data-action="zoom-out" title="缩小"></li>');
            htmlArrays.push('<li class="viewer-one-to-one" data-action="one-to-one" title="还原"></li>');
            htmlArrays.push('<li class="viewer-reset" data-action="reset" title="重置"></li>');
            htmlArrays.push('<li class="viewer-prev" data-action="prev" title="上一个"></li>');
            htmlArrays.push('\';if(tempJson && tempJson.isPlay){');
            htmlArrays.push('stringBufStr += \'<li class="viewer-play" data-action="play" title="放映幻灯片"></li>');
            htmlArrays.push('\';}');
            htmlArrays.push('stringBufStr += \'<li class="viewer-next" data-action="next" title="下一个"></li>');
            htmlArrays.push('<li class="viewer-rotate-left" data-action="rotate-left" title="逆时针旋转"></li>');
            htmlArrays.push('<li class="viewer-rotate-right" data-action="rotate-right" title="顺时针旋转"></li>');
            htmlArrays.push('<li class="viewer-flip-horizontal" data-action="flip-horizontal" title="水平翻转"></li>');
            htmlArrays.push('<li class="viewer-flip-vertical" data-action="flip-vertical" title="垂直翻转"></li>');
            htmlArrays.push('\';if(tempJson && tempJson.download){');
            htmlArrays.push('stringBufStr += \'<li class="viewer-download" data-action="download" title="下载单张图片"></li>\';');
            htmlArrays.push('stringBufStr += \'<li class="viewer-download-All" data-action="download-all" title="下载全部图片"></li>');
            htmlArrays.push('\';}');
            htmlArrays.push('stringBufStr += \'</ul>');
            htmlArrays.push('<div class="viewer-navbar">');
            htmlArrays.push('<ul class="viewer-list"></ul>');
            htmlArrays.push('</div>');
            htmlArrays.push('</div>');
            htmlArrays.push('<div class="viewer-tooltip"></div>');
            htmlArrays.push('<div class="viewer-button" data-action="mix"></div>');
            htmlArrays.push('<div class="viewer-player"></div>');
            htmlArrays.push('</div>');
            htmlArrays.push('</div>');
            htmlArrays.push('\' ; return stringBufStr;');
            return htmlArrays.join("");
        }()),
        u.other = i.fn.viewer,
        i.fn.viewer = function (e) {
            // 模板
            var temp = new Function('tempJson', u.TLATE);
            u.TEMPLATE = temp(i.extend({}, u.DEFAULTS, e));
