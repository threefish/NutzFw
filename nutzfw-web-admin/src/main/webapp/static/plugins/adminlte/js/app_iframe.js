        ...
        slimScroll: function (options) {
            ...
            this.each(function () {
                ...
                var isOverPanel, isOverBar, isDragg, queueHide, touchDif,
                    ...
                    divS = '<div></div>',
                    minBarHeight = 30,
                    releaseScroll = false, height = 50;
                // used in event handlers and for better minification
                var me = $(this);
                if (typeof o.height == "number") {
                    o.height = o.height - height;
                } else {
                    //解决无法滚动至底部
                    o.height = Number(o.height.replace("px", "")) - height;
                }
...
var addTabs = function (options) {
    ...
    if (options.url) {
        options.id = options.url.replace(/\//g, "_");
    }
    ...
    if (findTabPanel(pageId) === null) {
        ...
        if (options.content) {
            ...
            var $iframe = $("<iframe></iframe>").attr("src", options.url).addClass("tab_iframe")
                .attr("frameborder", "no")
...
function activeTabByPageId(pageId) {
    ...
    scrollToTab($title[0]);
    var $a = $("[tabs-id='" + pageId + "']");
    var $li = $("[tabs-id='" + pageId + "']").parent();
    var level = Number($li.attr("data-level"));
    $("li.treeview[data-level='0']").each(function () {
        $(this).removeClass("active");
    });
    $("li.treeview[data-level='" + level + "']").each(function () {
        $(this).removeClass("active");
    });
    $(".nav-link").removeClass("active");
    $(".menu-open").css("display", "none").removeClass("menu-open");
    $li.addClass("active");
    $a.addClass("active");
    if (level != 0) {
        var parentEl = $a;
        do {
            parentEl = parentEl.parent().parent();
            if (Number(parentEl.parent().attr("data-level")) == 0) {
                parentEl.prev(".nav-link").addClass("active");
            }
            parentEl.addClass("menu-open").css("display", "block");
            level--;
        } while (level > 0);
    }
}
    ...
    $.fn.sidebarMenu = function (options) {
        ...
        function init($menu_ul, data, level) {
            $.each(data, function (i, item) {
                ...
                //如果不是header
                var li = $('<li class="treeview menu-' + level + '" data-level="' + level + '"></li>');
