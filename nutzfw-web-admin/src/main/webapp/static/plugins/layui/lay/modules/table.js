...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function (exports) {
                        ...
                        var td = ['<td data-field="' + field + '" data-key="' + key + '" ' + function () { //追加各种属性
                            ...
                            , '<div class="layui-table-cell laytable-cell-' + function () { //返回对应的CSS类标识
                                ...
                                var tplData = $.extend(true, {
                                    //LAY_INDEX: numbers    2018年7月5日 16:25:05 黄川添加  在row data 中添加当前字段的field 辅助表格中附件的控制
                                    LAY_INDEX: numbers, LAY_FIELD: item3.field,
                                }, item1)
    ...
    table.reload = function (id, options) {
        var config = thisTable.config[id];
        //支持重载表头 黄川修改
        config.cols = options.cols == undefined ? config.cols : options.cols;
        options = options || {};
    ...
    //取得表格分页信息 黄川修改
    table.getTablePageConfig = function (id) {
        var config = thisTable.config[id];
        if (!config) return hint.error('The ID option was not found in the table instance');
        return config.page;
    };
