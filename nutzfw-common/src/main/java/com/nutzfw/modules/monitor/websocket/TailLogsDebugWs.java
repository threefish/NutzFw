 ...
 * 实时日志监控通道
 */
@ServerEndpoint(value = "/tailLogs/debug", configurator = NutWsConfigurator.class)
@IocBean
public class TailLogsDebugWs extends AbstractLogsWsSessionsEndpoint {

    @Inject("java:$conf.get('command.debug')")
    String command;

    @Override
    public WsHandler createHandler(Session httpSession, EndpointConfig config) {
        if (tailLogsWsHandler == null) {
            this.tailLogsWsHandler = new TailLogsWsHandler(command, "debug");
        }
